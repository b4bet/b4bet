    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display font-bold text-lg text-white">Lottery</h2>
          <button
          ...
          {([
            ...
            return (
              <button
                ...
                className={`group relative aspect-square rounded-2xl border-2 border-borderline-900 bg-slatepanel-900 overflow-hidden transition-all duration-200 ${g.border} hover:shadow-neon-glow active:scale-[0.97]`}
              >
                {logo ? (
                  <img
                    src={logo}
                  ...
                  <>
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
                      <Icon className="w-8 h-8 text-white drop-shadow-lg group-hover:scale-110 transition-transform" strokeWidth={1.5} />
      ...
      {/* Live stats strip — moved below the Lottery section */}
      <section className="grid grid-cols-3 gap-3">
        <div className="panel-tight p-3 text-center">
          ...
          <p className="tabular font-bold text-emeraldwin-400 text-lg">{stats.onlineUsers.toLocaleString()}</p>
        </div>
        <div className="panel-tight p-3 text-center">
          ...
          <p className="tabular font-bold text-neon-300 text-lg flex items-center justify-center gap-1">
            <TrendingUp className="w-3.5 h-3.5" /> {stats.topWin.toFixed(1)}x
          </p>
        </div>
        <div className="panel-tight p-3 text-center">
          <p className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Paid Out</p>
          <p className="tabular font-bold text-amberx-400 text-lg">₹{(stats.paidOut / 100000).toFixed(1)}M</p>
        </div>
      </section>

      {/* Trust strip — Provably Fair · Instant Payout · Secure Wallet */}
      <section className="grid grid-cols-3 gap-3">
        <div className="panel-tight p-3 text-center">
        ...
        <div className="panel-tight p-3 text-center">
          ...
          <p className="text-[10px] text-slate-400 font-semibold">Instant Payout</p>
        </div>
        <div className="panel-tight p-3 text-center">
