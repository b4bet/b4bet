...
export default function FinanceTab() {
  ...
  // Local UTR/reason store (not persisted to Supabase, shown in session only)
  const [localMeta, setLocalMeta]       = useState<Record<string, { utr?: string; reason?: string }>>({});

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [txns, betsData] = await Promise.all([
        supabaseGetTransactions(),
        supabaseGetBets(),
      ]);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">Live deposit, withdrawal and profit metrics — Supabase.</p>
        </div>
        <button
          onClick={() => void load()}
          disabled={loading}
      ...
      <div className="space-y-3">
        <h3 className="font-bold text-white flex items-center gap-2">
          <Banknote className="w-4 h-4 text-emerald-400" /> Deposit Requests
        </h3>
        {loading ? (
          ...
          <div className="space-y-2">
            {filteredTxns.filter((t) => t.type === 'deposit').map((t) => (
              <TxnCard
                key={t.id} t={t} acting={acting} setActing={setActing}
                inputVal={inputVal} setInputVal={setInputVal}
                updatingId={updatingId} localMeta={localMeta}
                onAccept={handleAccept} onComplete={handleComplete} onClear={clearAct}
              />
            ))}
          </div>
        )}
      ...
      <div className="space-y-3">
        <h3 className="font-bold text-white flex items-center gap-2">
          <TrendingDown className="w-4 h-4 text-red-400" /> Withdrawal Requests
        </h3>
        {loading ? (
          <div className="flex items-center justify-center p-8 text-slate-400 gap-2">
            <Loader2 className="w-5 h-5 animate-spin" /> Loading…
...
function StatCard({ label, value, icon: Icon, accent }: {
  label: string; value: string; icon: typeof TrendingUp; accent: string;
}) {
  return (
          ...
          <div className="flex gap-1.5">
            {t.status === 'pending' && (
              <button
                ...
                className="px-2 py-1 rounded-lg bg-blue-500/20 border border-blue-500/40 text-blue-300 text-[10px] font-semibold hover:text-blue-200"
              >
                Accept
              </button>
            )}
            {t.status === 'processing' && (
              <button
                onClick={() => { setActing({ id: t.id, mode: 'approve' }); setInputVal(''); }}
                className="px-2 py-1 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[10px] font-semibold hover:text-emerald-200"
              >
                Approve
              </button>
            )}
            {(t.status === 'pending' || t.status === 'processing') && (
              <button
                onClick={() => { setActing({ id: t.id, mode: 'cancel' }); setInputVal(''); }}
                className="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/40 text-red-300 text-[10px] font-semibold hover:text-red-200"
              >
                Reject
              </button>
            )}
      ...
      {isActing && (
        <div className="flex items-center gap-2">
          <input
            ...
            autoFocus
          />
          <button onClick={() => void onComplete()} className="px-3 py-2 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[10px] font-semibold hover:text-emerald-200 flex items-center gap-1">
