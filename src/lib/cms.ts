...
import { emailService, setEmailEnabled } from './emailService';
...
type EmailEnabledMap = Record<'welcome' | 'depositSuccess' | 'withdrawalStatus' | 'forgotPassword', boolean>;
...
class Cms {
  ...
  async syncSettingsFromSupabase() {
    try {
      ...
      // Load email_enabled map — applies on EVERY app load so admin toggle
      // takes effect across all users, not just inside the Email Manager tab.
      const emailEnabledRaw = find('email_enabled');
      if (emailEnabledRaw !== undefined && emailEnabledRaw !== null && typeof emailEnabledRaw === 'object') {
        const saved = emailEnabledRaw as Partial<EmailEnabledMap>;
        const keys = ['welcome', 'depositSuccess', 'withdrawalStatus', 'forgotPassword'] as const;
        keys.forEach(k => {
          if (saved[k] !== undefined) setEmailEnabled(k, saved[k] as boolean);
        });
      }
