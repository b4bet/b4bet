...
class Cms {
  ...
  private persistNotificationTemplatesToSupabase() {
    ...
    void supabase.rpc('admin_update_setting', { p_key: 'notification_templates', p_value: custom as unknown as string }).catch(() => {});
  ...
  updateBanner(id: string, patch: Partial<BannerSlide>) {
    ...
    if (patch.imageDataUrl || patch.linkUrl) {
      ...
      if (b) supabase.rpc('admin_upsert_banner', { p_id: id, p_title: 'Banner', p_image_url: b.imageDataUrl, p_link_url: b.linkUrl, p_sort_order: 0, p_is_active: true }).then(() => { void this.syncBannersFromSupabase(); }).catch(() => {});
    }
  }
  removeBanner(id: string) {
    this.banners = this.banners.filter(b => b.id !== id); this.emitBanners();
    supabase.rpc('admin_delete_banner', { p_id: id }).then(() => { void this.syncBannersFromSupabase(); }).catch(() => {});
  }
  ...
  setEmailTemplate(key: keyof EmailTemplates, html: string) {
    ...
    this.emitEmails();
    void supabase.rpc('admin_update_setting', { p_key: 'email_templates', p_value: this.emailTemplates as unknown as string }).catch(() => {});
  }
  ...
  private persistDynamicPagesToSupabase() {
    void supabase.rpc('admin_update_setting', { p_key: 'dynamic_pages', p_value: this.dynamicPages as unknown as string }).catch(() => {});
  }
