...
class Cms {
  ...
  addTicketMessage(ticketId: string, body: string, role: 'user' | 'agent', agentId?: string) {
    ...
    if (role === 'agent' && agentId) {
      rpc(supabase.rpc('admin_reply_ticket', { p_ticket_id: ticketId, p_staff_id: agentId, p_message: body }))
        .then(() => { void this.syncTicketsFromSupabase(); })
        .catch(() => { supabase.from('ticket_messages').insert({ ticket_id: ticketId, sender_type: 'staff', sender_id: agentId, message: body }).then(() => {}).catch(() => {}); });
    } else {
      const ticket = this.tickets.find(t => t.id === ticketId);
      if (ticket) {
        rpc(supabase.rpc('user_post_ticket_message', { p_account_id: ticket.accountId, p_body: body }))
          .then(() => { void this.syncTicketsFromSupabase(); })
          .catch(() => { supabase.from('ticket_messages').insert({ ticket_id: ticketId, sender_type: 'user', message: body }).then(() => {}).catch(() => {}); });
      }
    }
  }
  ...
  async loginStaff(email: string, password: string): Promise<StaffAccount | null> {
    const acc = await this.verifyStaffCredentialsAsync(email, password);
    if (acc) {
      this.staffSessionId = acc.id;
      try { localStorage.setItem(ADMIN_SESSION_KEY, acc.id); } catch { /* ignore */ }
    }
    return acc;
  }
  ...
  async saveSmtpConfig(config: SmtpConfig) {
    this.smtpConfig = { ...config };
    const fields: Array<[string, unknown]> = [
      ['smtp_host', config.host], ['smtp_port', config.port],
      ['smtp_user', config.user], ['smtp_pass', config.pass],
      ['smtp_tls', config.tls], ['smtp_active', config.active],
    ];
    await Promise.all(fields.map(([k, v]) =>
      rpc(supabase.rpc('admin_update_setting', { p_key: k, p_value: v as unknown as string })).catch(() => {})
    ));
  }
  ...
  async saveSiteLogo(dataUrl: string | null) {
    this.logoDataUrl = dataUrl; this.emitLogo();
    await rpc(supabase.rpc('admin_update_setting', { p_key: 'site_logo_data_url', p_value: (dataUrl ?? '') as unknown as string })).catch(() => {});
  }

  async saveSiteTextLogo(dataUrl: string | null) {
    this.textLogoDataUrl = dataUrl; bus.emit(Topics.TextLogo, this.textLogoDataUrl);
    await rpc(supabase.rpc('admin_update_setting', { p_key: 'site_text_logo_data_url', p_value: (dataUrl ?? '') as unknown as string })).catch(() => {});
  }

  async saveSiteFavicon(dataUrl: string | null) {
    this.faviconDataUrl = dataUrl; bus.emit(Topics.Favicon, this.faviconDataUrl);
    await rpc(supabase.rpc('admin_update_setting', { p_key: 'site_favicon_data_url', p_value: (dataUrl ?? '') as unknown as string })).catch(() => {});
  }
