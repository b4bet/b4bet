...
// Default notification templates (used as baseline when Supabase has no saved state)
const DEFAULT_NOTIFICATION_TEMPLATES: NotificationTemplate[] = [
  { id: 'nt_welcome', title: 'Welcome!', body: 'Account created. Welcome aboard!', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_login', title: 'Logged In', body: 'Welcome back! Your session is now active.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_logout', title: 'Logged Out', body: 'Your session has ended. See you next time!', kind: 'info', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_password_reset', title: 'Password Reset', body: 'Your password has been updated successfully.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_password_changed', title: 'Password Changed', body: 'Your password was updated successfully.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_deposit_ok', title: 'Deposit Confirmed', body: 'Your deposit has been approved.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_withdrawal_ok', title: 'Withdrawal Processed', body: 'Your withdrawal request has been processed.', kind: 'info', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_withdrawal_refunded', title: 'Withdrawal Refunded', body: 'Your withdrawal was rejected. Amount refunded to your wallet.', kind: 'warn', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_referral_reward', title: 'Referral Reward', body: 'You earned a bonus from a referral deposit!', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_pending_rewards', title: 'Pending Rewards Credited', body: 'Pending referral rewards have been added to your balance.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_redeem', title: 'Redeem Code Applied', body: 'Your promo code unlocked bonus credits.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_promo', title: 'Special Offer', body: 'Exclusive promo available! Check the promotions page.', kind: 'warn', isActive: false, isAutoGenerated: false, createdAt: 0 },
  { id: 'nt_profile_updated', title: 'Profile updated', body: 'Your contact info was saved.', kind: 'success', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_cashout_failed', title: 'Cashout failed', body: 'Your cashout could not be completed.', kind: 'warn', isActive: true, isAutoGenerated: true, createdAt: 0 },
  { id: 'nt_mines_failed', title: 'Mines failed', body: 'Your Mines action could not be completed.', kind: 'warn', isActive: true, isAutoGenerated: true, createdAt: 0 },
];
...
function mapPaymentMethod(row: Record<string, unknown>): ManualMethod {
  ...
  // qrDataUrl: admin may store as 'qrDataUrl' (legacy) or 'qrImageUrl' (PaymentMethodsTab upload)
  const qrDataUrl = (details.qrDataUrl || details.qrImageUrl) as string | undefined;
  return {
    id: row.id as string,
...
class Cms {
  ...
  // Notification templates — seeded from defaults, then overridden by Supabase on load.
  // ALL templates (auto-generated + custom) are saved to Supabase so toggle/edit
  // changes made by admin reach every user's browser on next load.
  private _notificationTemplates: NotificationTemplate[] = DEFAULT_NOTIFICATION_TEMPLATES.map(t => ({ ...t, createdAt: Date.now() }));
  ...
  async syncSettingsFromSupabase() {
    try {
      ...
      // Load ALL notification templates from Supabase.
      // Merge strategy: Supabase is the source of truth for each template's
      // isActive / title / body / kind. For any id not in Supabase, keep the default.
      const notifTpls = find('notification_templates');
      if (notifTpls !== undefined && notifTpls !== null && Array.isArray(notifTpls)) {
        const saved = notifTpls as NotificationTemplate[];
        const savedById = new Map(saved.map(t => [t.id, t]));
        // Start from defaults and override with saved values
        const merged = DEFAULT_NOTIFICATION_TEMPLATES.map(def => {
          const s = savedById.get(def.id);
          return s ? { ...def, ...s } : def;
        });
        // Append any purely custom templates (not in defaults) from Supabase
        for (const s of saved) {
          if (!merged.find(m => m.id === s.id)) merged.push(s);
        }
        this._notificationTemplates = merged;
  ...
  // Persist ALL templates (auto-generated + custom) to Supabase so that
  // toggle off/on, title edits, and body edits reach every user's browser.
  private persistNotificationTemplatesToSupabase() {
    void rpc(supabase.rpc('admin_update_setting', { p_key: 'notification_templates', p_value: this._notificationTemplates as unknown as string })).catch(() => {});
  }
  ...
  // Check template isActive before pushing — if off, skip the notification entirely.
  pushFromTemplate(templateId: string, fallbackTitle: string, fallbackBody: string, fallbackKind: NotificationTemplateKind = 'info') {
    ...
    if (tpl) {
      // Template found — only push if it is active
      if (tpl.isActive) store.pushNotification({ title: tpl.title, body: tpl.body, kind: tpl.kind });
      return;
    }
    // Template not found at all — use fallback
  ...
  async updateStaffPermissions(id: string, permissions: Partial<Record<PermissionKey, boolean>>) {
    await supabase.rpc('admin_update_staff_permissions', { p_id: id, p_permissions: permissions as unknown as Record<string, boolean> });
    await this.syncStaffFromSupabase();
  }

  async updateStaffPassword(id: string, newPassword: string) {
    await supabase.rpc('admin_update_staff_password', { p_id: id, p_password: newPassword });
  }

  async creditBalance(username: string, amount: number) {
    const { error } = await supabase.rpc('admin_credit_balance', { p_username: username, p_amount: amount });
    if (error) throw error;
    await this.syncUsersFromSupabase();
  }

  async debitBalance(username: string, amount: number) {
    const { error } = await supabase.rpc('admin_debit_balance', { p_username: username, p_amount: amount });
    if (error) throw error;
    await this.syncUsersFromSupabase();
  }

  async banUser(userId: string, reason?: string) {
    await supabase.rpc('admin_ban_user', { p_id: userId, p_reason: reason ?? null });
    await this.syncUsersFromSupabase();
  }

  async unbanUser(userId: string) {
    await supabase.rpc('admin_unban_user', { p_id: userId });
    await this.syncUsersFromSupabase();
  }
  ...
  async syncReferralsFromSupabase() { return this.loadReferrals(); }
  ...
  async syncAffiliatesFromSupabase() { return this.loadAffiliates(); }
  ...
  getAffiliateByUserId(userId: string): AffiliateApplication | undefined {
    return this.affiliates.find(a => a.userId === userId);
  }

  async approveAffiliate(id: string, revSharePct: number) {
    this.affiliates = this.affiliates.map(a => a.id === id ? { ...a, status: 'approved' as const, revSharePct } : a);
    bus.emit(Topics.Affiliates, this.affiliates);
    await supabase.rpc('admin_approve_affiliate', { p_id: id, p_rev_share_pct: revSharePct }).catch(() => {});
  }

  async rejectAffiliate(id: string) {
    this.affiliates = this.affiliates.map(a => a.id === id ? { ...a, status: 'rejected' as const } : a);
    bus.emit(Topics.Affiliates, this.affiliates);
    await supabase.rpc('admin_reject_affiliate', { p_id: id }).catch(() => {});
  }
  ...
  async updateUserProfile(userId: string, patch: { username?: string; email?: string; phone?: string; vipLevel?: number; isActive?: boolean }) {
    ...
    this.adminUsers = this.adminUsers.map(u =>
      u.id === userId ? { ...u, ...(patch.username !== undefined ? { username: patch.username } : {}), ...(patch.vipLevel !== undefined ? { vipLevel: patch.vipLevel } : {}) } : u,
    );
    bus.emit(Topics.AdminUsers, this.adminUsers);
  }
}
...
(cms as unknown as { _authUser: AuthUser | null })._authUser = null;
