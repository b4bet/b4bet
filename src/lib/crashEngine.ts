// crashEngine.ts — Server-side bust point via process-bet Edge Function
//
// The bust point is fetched at round START from the Supabase Edge Function
// which uses crypto.getRandomValues() server-side. The client NEVER computes
// the bust point — it's purely a display layer.
//
// If the edge function fails, a backup local RNG kicks in so the game
// never freezes, but the server is always authoritative for settlements.
...
function freshBet(id: 'A' | 'B'): BetSlot {
  return { id, amount: 100, placed: false, autoCashAt: null, cashedOutAt: null, win: null };
}
...
class CrashEngine {
      ...
      if (this.state.countdown <= 0) {
        if (!this.bustPointReceived) {
          ...
          if (waited > SERVER_TIMEOUT_MS) {
            console.warn('[CrashEngine] Server bust timeout, backup RNG');
    ...
    GameService.crashGetBustPoint(roundId)
      .then((res) => {
        if (this.state.roundId === roundId && this.state.phase !== 'busted') {
          this.bustPointReceived = true;
          this.state.bustPoint = Math.max(1.10, res.bust_point);
        }
      })
      .catch((err) => {
        console.warn('[CrashEngine] Server bust failed r', roundId, err?.message ?? err);
      });
  }
  ...
  private performCashOut(id: 'A' | 'B', cashOutAt: number) {
    ...
    store.recordCrashBet({
      roundId: this.state.roundId, amount: slot.amount,
      cashOutAt, bustPoint: this.state.bustPoint, win: slot.win ?? 0,
    });
  }
  ...
  private settleBustedBets() {
    for (const slot of Object.values(this.state.bets)) {
      ...
      store.recordCrashBet({
        roundId: this.state.roundId, amount: slot.amount,
        cashOutAt: null, bustPoint: this.state.bustPoint, win: 0,
      });
    }
    this.broadcastBets();
  }
  ...
  getState(): CrashState {
    ...
    return {
      phase, multiplier, countdown, roundId, bustPoint, history,
