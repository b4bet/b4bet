...
// ─── Fallback SVG logo (shown if admin hasn't uploaded a custom logo) ──────────
...
function SplashScreen() {
  // Read admin-uploaded logo from cms (same source as Header uses)
  const [logoUrl, setLogoUrl] = useState<string>(() => cms.logoDataUrl);

  useEffect(() => {
    // Sync immediately in case cms loaded before component mounted
    setLogoUrl(cms.logoDataUrl);
    // Also listen for live updates
    return bus.on(Topics.Logo, (url) => setLogoUrl(url as string));
  }, []);

  return (
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#020617]">
      {/* Logo — admin uploaded image or fallback SVG */}
...
export default function App() {
  ...
  // Dynamic favicon: listen to cms:favicon bus event and update <link rel="icon">
  useEffect(() => {
